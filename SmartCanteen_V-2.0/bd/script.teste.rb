...
hash = Digest::SHA256.hexdigest("1234")
...
begin
  DB.transaction do
    ...
    10.times do |i|
      DB.execute("INSERT INTO funcionarios (nome, CPF, telefone, cargos_idcargos, senha_hash) VALUES (?, ?, ?, ?, ?)", 
                 ["Funcionario #{i+1}", "123.456.789-#{i}#{i}", "119888#{i}#{i}", i + 1, hash])
    end
